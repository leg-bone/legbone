...
export const controls = (function() {
  ...
  class _ShipControls {
    ...
    Update(timeInSeconds) {
      ...
      if (this._move.forward) {
        _A.set(1, 0, 0);
        _Q.setFromAxisAngle(_A, -Math.PI * timeInSeconds * .25 * this._acceleration.y);
        _R.multiply(_Q);
      }
      if (this._move.backward) {
        _A.set(1, 0, 0);
      ...
      if (this._move.left) {
        _A.set(0, 1, 0);
        _Q.setFromAxisAngle(_A, Math.PI * timeInSeconds * .5 * this._acceleration.y);
        _R.multiply(_Q);
      }
      if (this._move.right) {
        _A.set(0, 1, 0);
      ...
      if (this._move.rocket) {
        velocity.z -= this._acceleration.x * 5 * timeInSeconds;
      }
